...
class _CallbackContainerBase:
    ...
    def _ingest_callback(self, callback, button=None, key=None, double_click=None):
        ...
        d = self._cbs.setdefault(ds, dict()).setdefault(button_modifier, dict())
    ...
    def remove(self, cid):
        ...
        if (c0 := self._cbs.get(clicktype, None)):
            if (c1 := c0.get(f"{button}__{key}", None)):
                if f"{name}__{layer}" in c1:
                    c1.pop(f"{name}__{layer}")
                    return
        raise _log.error(f"Callback ID {cid} not found for {self._method} callbacks")
    ...
    def _execute_cbs_for_event(self, event):
        """
        ...
        self._event = event
        double_click = getattr(event, "dblclick", None)
        key = getattr(event, "key", None)
        button = getattr(event, "button", None)

        # get callbacks to execute based on single/double click property
        cbs = [self._cbs.get("any", dict())]
        if double_click is True:
            cbs.append(self._cbs.get("double", dict()))
        elif double_click is False:
            cbs.append(self._cbs.get("single", dict()))
        ...
        for c in cbs:
            if button_modifier in c:
                bcbs = c[button_modifier]
