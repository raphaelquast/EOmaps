...
class Maps(object):
    ...
    def new_axes(self, ax=None, layer=None, dynamic=True, **kwargs):
        ...
        Note
        ----
        EOmaps tries its best to avoid unnecessary re-draws of layers.
        Since re-draws are triggered by maps and not by ordinary matplotlib
        axes, it can happen that artists of the axes are not immediately drawn.

        To manually trigger a re-draw of the figure, call `m.redraw()`.
        (or simply click on any map in the figure)
        ...
        if np.allclose(self.ax.bbox.bounds, ax.bbox.bounds):
            print(
                "EOmaps: Warning! The new axes overlap exactly with the parent map! "
                "Use `ax=...` or the LayoutEditor to adjust the position of the axes."
            )

        if layer is None:
            layer = self.layer

        self.BM.add_artist(ax, layer)
