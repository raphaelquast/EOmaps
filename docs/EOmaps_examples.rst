...
🛸 Turn your plot into a powerful widget
-----------------------------------------
...
.. code-block:: python
    ...
    s3 = m.add_scalebar(-73, 8, 0, scale=500000,
...
.. code-block:: python
    ...
    lon, lat = np.meshgrid(np.linspace(lonmin, lonmax, Nlon),
                        np.linspace(latmin, latmax, Nlat))
    ...
    mg.m_top.set_classify_specs(scheme=Maps.CLASSIFIERS.UserDefined,
    ...
    def update_plots(self, ID, **kwargs):
        ...
        cnt, val, art = mg.ax_right.hist(x.values, bins=50,
                                        orientation="horizontal", fc="C0")
